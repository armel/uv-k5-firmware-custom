...
| DMA, CRC, UART0/2, flash controller, PWM_PLUS0, SPI1, AES, PMU | Every one of these has a real base address in `bsp/dp32g030/*.h` and is tagged as unimplemented in `sysbus: init:` — confirmed never touched by this firmware by direct source inspection, so real modeling would be wasted effort. This is the complete peripheral list from every `bsp/dp32g030/*.h` file cross-checked against this platform description; nothing with a real address is left unmapped. |
| IIC0/IIC1, TIMER, RTC, watchdogs (WWDT/IWDT), CMP | **Not peripherals with addresses in this BSP at all** — no header defines registers or a base address for any of them, and there's zero reference to them anywhere in this codebase, so firmware couldn't touch them even if it wanted to (nothing would compile). The one partial exception is the watchdog: `SYSCON_DEV_CLK_GATE` has real WWDT/IWDT clock-gate bits (bits 24/23), but `main.c`'s own `SYSCON_DEV_CLK_GATE = ...` assignment explicitly excludes both, so the watchdog is provably always clock-gated off — not modeled, and correctly so. |
